...
function build_model(; name, use_mg=false, simplify=true, bcl=1second, istim=-80μAcm⁻², tend=1000second, tstart=zero(tend), duty=0.5ms)
    @parameters begin
        ...
        ΣNAD_m = 10mM           # Mitochondrial NAD + NADH pool (Zhou, 2009) # Or 1.0mM (Gauthier, 2013)
    ...
    eqs = [
        ...
        D(ca_i) ~ β_ca(ca_i, KM_CA_CMDN, ET_CMDN) * (Jxfer - Jup - Jtrpn - 0.5A_CAP_V_MYO_F * (IPMCA + ICaB - 2 * INaCa) + V_MITO_V_MYO * (vNaCa - vUni)),
        D(ca_nsr) ~ (V_MYO * Jup - V_JSR * Jtr) / V_NSR,
        D(ca_jsr) ~ β_ca(ca_jsr, KM_CA_CSQN, ET_CSQN) * (Jtr - Jrel),
        D(ca_ss) ~ β_ca(ca_ss, KM_CA_CMDN, ET_CMDN) * ((V_JSR * Jrel - V_MYO * Jxfer) / V_SS - ICaL * 0.5A_CAP_V_SS_F),
        ...
        D(dpsi) ~ iCM_MITO * (vHres - vHu - vANT - vHleak - vNaCa - 2vUni - vIMAC),
        D(sox_m) ~ vROS - vTrROS,
        ΣA_i ~ atp_i + adp_i,
        ΣA_m ~ atp_m + adp_m,
        ΣNAD_m ~ nad_m + nadh_m,
