...
function get_ros_sys(; dpsi, sox_m, nadph_i=75μM, V_MITO_V_MYO=0.615, name=:rossys)
    @parameters begin
        ...
        KM_SOD = 5μM    # Michaelis constant of SOD  # 20.2nM (Zhou, 2009)
        K_SOD = 4 / ms  # Catalytic constant of SOD  # 24.48Hz (Zhou, 2009)
        KI_H2O2_SOD = 500μM         # H2O2 inhibition constant of SOD
        ET_SOD_I = 1.43μM           # Cytosolic SOD concentration # 1.43μM in (Zhou, 2009)
    ...
    @variables begin
        ...
        E_ROS(t)    # Reversal potential of ROS
    end

    eqs = [
        E_ROS ~ nernst(sox_i, sox_m, -1),
        vTrROS ~ J_IMAC * gIMAC * (dpsi + E_ROS),
        vIMAC ~ gIMAC * dpsi,
        gIMAC ~ fvIMAC * faIMAC,
        ...
        vSOD_i ~ K_SOD * ET_SOD_I * hil(sox_i, KM_SOD) * hil(KI_H2O2_SOD, h2o2_i),
