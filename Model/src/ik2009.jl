function get_ik_eqs(; na_i, na_o, k_i, k_o, vm, atp_i, adp_i, mg_i=1mM, EK=nernst(k_o, k_i))
    fko = sqrt(k_o / 5.4mM)
    @parameters begin
    ...
    eqs_ik = [
        D(x_k) ~ αk - x_k * (αk + βk),
        αk1 ~ 1.02 / (1 + exp(0.2385/mV * (vk - 59.215mV))),
        βk1 ~ (0.4912 * exp(0.08032/mV * (vk + 5.476mV)) + exp(0.06175/mV * (vk - 594.31mV))) / (1 + exp(-0.5143/mV * (vk + 4.753mV))),
        αk ~ 7.19e-5 / ms / (0.148) * exprel(-0.148/mV * (vm + 30mV)),
        βk ~ 1.31e-4 / ms / (0.0687) * exprel(0.0687/mV * (vm + 30mV)),
        x1k ~ expit(-inv(40mV) * (vm - 40mV)),
        fkatp ~ hil(km_atp, atp_i, hatp),
        IK1 ~ G_K1 * (αk1 / (αk1 + βk1)) * ΔVK,
        IK ~ G_K * x1k * x_k^2 * (vm - E_KNa),
        IKp ~ G_KP * ΔVK / (1 + exp(-(vm - 7.488) / 5.98)),
        IKatp ~ G0_KATP * fkatp * ΔVK
    ]
